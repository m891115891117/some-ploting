...
for(i in 1:length(files)){
  ...
  #assign(paste("bin",i,sep = "\t"),meltMean[[i]])
  #names(meltMeantmp[[i]])[1]<-"bin"
  #names(meltMeantmp[[i]])[2]<- paste0("chr",i)
  
   # aa<-as.data.frame(rbind(aa,i))
   # bb <- as.data.frame(rbind(bb,i,meltMean[[i]][meltMean[[i]][2]>200,]))
   # cc <-as.data.frame(rbind(cc,i,meltMean[[i]][meltMean[[i]][2]>0,]))   
   # 
...
# sv <- read.table("chrYouttest.txt",header = TRUE,check.names = FALSE)
# 
# meltsv <- melt(sv)
# names(meltsv)[2:3] <- c("bin", "SV")
# mm<-aggregate(meltsv$SV, by=list(meltsv$bin), FUN=sum)
# names(mm)<- c("bin","mean")
# ggplot(mm,aes(x=bin,y=mean,group=1))+
#   geom_line(colour="blue",size=0.2)+
#   theme(axis.text.x = element_blank(),
#         axis.text.y = element_blank(),
#         axis.title = element_blank(),
...
#         panel.background=element_blank(),
#         plot.margin = unit(c(3,0,0,0),"mm"))+
#   scale_y_continuous(name = "Depth",limits=c(0, 360),breaks = seq(0, 360, 20))
